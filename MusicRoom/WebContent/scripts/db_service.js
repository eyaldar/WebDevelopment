...
(function() {
	...
	var openDB = function() {
		try {
			if (window.openDatabase) {
				...
				// test query to check if the db exists. if not - it goes to the
				// error callback and creates it
				executeQuery('select * from MY_DATA', function() {
				}, createDB, true);
			}
		} catch (e) {
			alert(e);
	...
	var createDB = function() {
		...
		var createTableQueries = {
			...
			userTypes : 'CREATE TABLE USER_TYPES('
					+ 'ID 			INTEGER PRIMARY KEY,'
					+ 'DESCRIPTION 	TEXT NOT NULL);',

			roomTypes : 'CREATE TABLE ROOM_TYPES('
					+ 'ID 				INTEGER PRIMARY KEY,'
					+ 'ROOM_TYPE_NAME 	TEXT NOT NULL);',

			equipmentCategories : 'CREATE TABLE EQUIPMENT_CATEGORIES('
					+ 'ID 				INTEGER PRIMARY KEY,'
					+ 'EQUIP_CAT_NAME 	TEXT NOT NULL);',

			equipmentTypes : 'CREATE TABLE EQUIPMENT_TYPES('
					+ 'ID 				INTEGER PRIMARY KEY,'
					+ 'EQUIPMENT_NAME 	TEXT NOT NULL,'
					+ 'CATEGORY_ID 		INTEGER NOT NULL,'
					+ 'Foreign Key (CATEGORY_ID) references EQUIPMENT_CATEGORIES(ID) on delete cascade);',

			myData : 'CREATE TABLE MY_DATA('
					+ 'MY_ID 			INTEGER,'
			...
			roomRoomTypes : 'CREATE TABLE ROOM_ROOM_TYPES('
			...
			memberInstruments : 'CREATE TABLE MEMBER_INSTRUMENT('
			...
			roomEquipment : 'CREATE TABLE ROOM_EQUIPMENT('
					+ '    ROOM_ID				INTEGER NOT NULL,'
					+ '	   EQUIPMENT_TYPE_ID 	INTEGER NOT NULL,'
					+ '	   MODEL	 			TEXT NOT NULL,'
			...
			myQueries : 'CREATE TABLE MY_QUERIES('
		...
		db.transaction(function(tx) {
			...
			// Create tables
			for ( var query in createTableQueries) {
				if (createTableQueries.hasOwnProperty(query)) {
			...
			// Create indices
			for ( var query in createIndicesQueries) {
				if (createTableQueries.hasOwnProperty(query)) {
					tx.executeSql(query, [], function(tx, result) {
					}, function(tx, error) {
					});
	...
	var populateDecodeTables = function() {
		...
		areasService.getList().then(
				function(areas) {
					for (var i = 0; i < areas.length; i++) {
						executeQuery("insert into AREAS values " + areas[i].id
								+ "," + areas[i].name, null, null, true);

						var citiesService = Restangular.all("cities",
								areas[i].id);

						citiesService.getList().then(
								function(cities) {
		...
		room_typesService.getList().then(
				function(types) {
					for (var i = 0; i < types.length; i++) {
						executeQuery("insert into ROOM_TYPES values "
								+ types[i].id + "," + types[i].name, null,
								null, true);
					}
				});
		...
		user_typesService.getList().then(
				function(types) {
					for (var i = 0; i < types.length; i++) {
						executeQuery("insert into USER_TYPES values "
								+ types[i].id + "," + types[i].name, null,
								null, true);
					}
				});

		var equipment_categoriesService = Restangular
				.all("equipment_categories");

		equipment_categoriesService.getList().then(
				function(cats) {
					for (var i = 0; i < cats.length; i++) {
						executeQuery("insert into EQUIPMENT_CATEGORIES values "
								+ cats[i].id + "," + cats[i].name, null, null,
								true);

						var equipment_typesService = Restangular.all(
								"equipment_types", cats[i].id);

						equipment_typesService.getList().then(
								function(types) {
	...
	musicRoom.factory("db_service", [ function() {
		...
		var factory = {
			openDB : openDB,
			createDB : createDB,
			populateDB : populateDecodeTables,
