		...
		db
				.transaction(function(tx) {
					tx.executeSql('CREATE TABLE AREAS('
							...
							+ 'AREA_NAME 	TEXT NOT NULL);', [], function(tx,
							result) {
					}, function(tx, error) {
					});
					...
					tx.executeSql('CREATE TABLE USER_TYPES('
							+ 'ID 			INTEGER PRIMARY KEY,'
							+ 'DESCRIPTION 	TEXT NOT NULL);', [], function(tx,
							result) {
					}, function(tx, error) {
					});
					...
					tx.executeSql('CREATE TABLE ROOM_TYPES('
							+ 'ID 				INTEGER PRIMARY KEY,'
							+ 'ROOM_TYPE_NAME 	TEXT NOT NULL);', [], function(
							tx, result) {
					}, function(tx, error) {
					});
					...
					tx.executeSql('CREATE TABLE EQUIPMENT_CATEGORIES('
							+ 'ID 				INTEGER PRIMARY KEY,'
							+ 'EQUIP_CAT_NAME 	TEXT NOT NULL);', [], function(
							tx, result) {
					}, function(tx, error) {
					});
					...
					tx.executeSql('CREATE TABLE BANDS('
							...
							+ '	   BAND_NAME 	TEXT NOT NULL,'
							+ '	   LOGO_URL	 	TEXT NOT NULL,'
							+ '	   GENRE	 	TEXT NOT NULL,', [], function(tx,
							result) {
					}, function(tx, error) {
					});
					...
					tx
							.executeSql(
									'CREATE INDEX STD_CT_IDX 		ON STUDIOS 			(CITY_ID)',
									[], function(tx, result) {
									}, function(tx, error) {
									});
					tx.executeSql(
							'CREATE INDEX CT_AREA_IDX 	ON CITIES 			(AREA_ID)',
							[], function(tx, result) {
							}, function(tx, error) {
							});
					tx
							.executeSql(
									'CREATE INDEX STD_NAME_IDX 	ON STUDIOS 			(STUDIO_NAME)',
									[], function(tx, result) {
									}, function(tx, error) {
									});
					tx
							.executeSql(
									'CREATE INDEX ROM_STD_IDX 	ON ROOMS 			(STUDIO_ID)',
									[], function(tx, result) {
									}, function(tx, error) {
									});
					tx.executeSql(
							'CREATE INDEX ROM_RATE_IDX 	ON ROOMS 			(RATE)',
							[], function(tx, result) {
							}, function(tx, error) {
							});
					tx
							.executeSql(
									'CREATE INDEX EQP_CAT_IDX 	ON EQUIPMENT_TYPES 	(CATEGORY_ID)',
									[], function(tx, result) {
									}, function(tx, error) {
									});
